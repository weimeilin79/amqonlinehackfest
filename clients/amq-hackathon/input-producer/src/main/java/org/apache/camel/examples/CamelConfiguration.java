...
public class CamelConfiguration extends RouteBuilder {
  ...
  @Bean
  @Scope("prototype")
  private byte[] randomBytes(AmqpConfiguration amqpProperties) {
    Random rnd = new Random();
    int offsetMaxBytes = amqpProperties.getMaxMessageSize() - amqpProperties.getMinMessageSize();
    int randomSize = rnd.nextInt(offsetMaxBytes);
    byte[] bytes = new byte[rnd.nextInt(randomSize)];
    rnd.nextBytes(bytes);
    return bytes;
  }
  ...
  public void configure() throws Exception {
    fromF("timer:kickstartmyroute?period=%s", amqpProperties.getTimerPeriod())
      .setHeader("ClientID", constant(amqpProperties.getClientId()))
      .setBody(simple("ref:randomBytes"))
      .log(LoggingLevel.DEBUG, log, "ClientID: ${header.ClientID}, Message: ${bodyAs(java.lang.String).length()}")
      .toF("amqp:queue:%s", amqpProperties.getProducerQueue())
    ;
  }
}
